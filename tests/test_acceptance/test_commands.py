...
Runtime: ~10-15 minutes (real RF processing per command).
...
class TestAOCSCommandsRF:
    ...
    def test_cmd_aocs_set_mode_off(self, harness):
        r = harness.send_s8(0, bytes([0]), "AOCS_MODE_OFF")
        assert r["ack_11"], "No S1.1 for AOCS mode OFF"

    def test_cmd_aocs_set_mode_detumble(self, harness):
        r = harness.send_s8(0, bytes([2]), "AOCS_MODE_DETUMBLE")
        assert r["ack_11"], "No S1.1 for AOCS mode DETUMBLE"

    def test_cmd_aocs_set_mode_coarse_sun(self, harness):
        r = harness.send_s8(0, bytes([3]), "AOCS_MODE_COARSE_SUN")
        assert r["ack_11"], "No S1.1 for AOCS mode COARSE_SUN"

    def test_cmd_aocs_set_mode_nominal(self, harness):
        r = harness.send_s8(0, bytes([4]), "AOCS_MODE_NOMINAL")
        assert r["ack_11"], "No S1.1 for AOCS mode NOMINAL"

    def test_cmd_aocs_set_mode_fine_point(self, harness):
        r = harness.send_s8(0, bytes([5]), "AOCS_MODE_FINE_POINT")
        assert r["ack_11"], "No S1.1 for AOCS mode FINE_POINT"

    def test_cmd_aocs_set_mode_desat(self, harness):
        r = harness.send_s8(0, bytes([7]), "AOCS_MODE_DESAT")
        assert r["ack_11"], "No S1.1 for AOCS mode DESAT"

    def test_cmd_aocs_desaturate(self, harness):
        r = harness.send_s8(1, name="AOCS_DESATURATE")
        assert r["ack_11"]

    def test_cmd_aocs_disable_wheel_0(self, harness):
        r = harness.send_s8(2, bytes([0]), "AOCS_DISABLE_WHEEL_0")
        assert r["ack_11"]

    def test_cmd_aocs_enable_wheel_0(self, harness):
        r = harness.send_s8(3, bytes([0]), "AOCS_ENABLE_WHEEL_0")
        assert r["ack_11"]

    def test_cmd_aocs_st1_power_on(self, harness):
        r = harness.send_s8(4, bytes([1]), "ST1_POWER_ON")
        assert r["ack_11"]

    def test_cmd_aocs_st2_power_on(self, harness):
        r = harness.send_s8(5, bytes([1]), "ST2_POWER_ON")
        assert r["ack_11"]

    def test_cmd_aocs_st_select_2(self, harness):
        r = harness.send_s8(6, bytes([2]), "ST_SELECT_2")
        assert r["ack_11"]

    def test_cmd_aocs_mag_select(self, harness):
        r = harness.send_s8(7, bytes([1]), "MAG_SELECT")
        assert r["ack_11"]

    def test_cmd_aocs_mtq_enable(self, harness):
        r = harness.send_s8(9, bytes([1]), "MTQ_ENABLE")
        assert r["ack_11"]

    def test_cmd_aocs_gyro_calibration(self, harness):
        r = harness.send_s8(13, name="GYRO_CALIBRATION")
        assert r["ack_11"]

    def test_cmd_aocs_set_deadband(self, harness):
        r = harness.send_s8(15, struct.pack('>f', 0.5), "SET_DEADBAND_0.5")
        assert r["ack_11"]

    def test_cmd_aocs_verify_hk(self, harness):
        """Verify AOCS HK (SID 2) is recoverable through RF after commanding."""
        hk = harness.get_hk(2)
        assert hk is not None, "No AOCS HK recovered through RF"
        assert hk["size"] > 10, "AOCS HK too small"
...
class TestEPSCommandsRF:
    ...
    def test_cmd_eps_power_on_aocs(self, harness):
        r = harness.send_s8(19, bytes([7]), "PWR_ON_AOCS_WHEELS")
        assert r["ack_11"]

    def test_cmd_eps_power_off_aocs(self, harness):
        r = harness.send_s8(20, bytes([7]), "PWR_OFF_AOCS_WHEELS")
        assert r["ack_11"]
        # Re-enable for subsequent tests
        harness.send_s8(19, bytes([7]), "PWR_ON_AOCS_WHEELS_RESTORE")

    def test_cmd_eps_power_on_payload(self, harness):
        r = harness.send_s8(19, bytes([4]), "PWR_ON_PAYLOAD")
        assert r["ack_11"]

    def test_cmd_eps_power_on_htr_bat(self, harness):
        r = harness.send_s8(19, bytes([5]), "PWR_ON_HTR_BAT")
        assert r["ack_11"]

    def test_cmd_eps_power_on_ttc_tx(self, harness):
        r = harness.send_s8(19, bytes([3]), "PWR_ON_TTC_TX")
        assert r["ack_11"]

    def test_cmd_eps_reset_oc_flag(self, harness):
        r = harness.send_s8(21, bytes([3]), "RESET_OC_FLAG_3")
        assert r["ack_11"]

    def test_cmd_eps_emergency_load_shed(self, harness):
        r = harness.send_s8(25, bytes([1]), "EMERGENCY_LOAD_SHED_1")
        assert r["ack_11"]

    def test_cmd_eps_deploy_wing(self, harness):
        r = harness.send_s8(81, bytes([2]), "DEPLOY_WING_BOTH")
        assert r["ack_11"]

    def test_cmd_eps_set_charge_rate(self, harness):
        r = harness.send_s8(23, struct.pack('>f', 2.0), "SET_CHARGE_RATE_2A")
        assert r["ack_11"]

    def test_cmd_eps_set_payload_mode(self, harness):
        r = harness.send_s8(16, bytes([0]), "PAYLOAD_MODE_OFF")
        assert r["ack_11"]

    def test_cmd_eps_fpa_cooler(self, harness):
        r = harness.send_s8(17, bytes([1]), "FPA_COOLER_ON")
        assert r["ack_11"]

    def test_cmd_eps_verify_hk(self, harness):
        """Verify EPS HK (SID 1) through RF."""
        hk = harness.get_hk(1)
...
class TestPayloadCommandsRF:
    ...
    def test_cmd_pld_mode_standby(self, harness):
        r = harness.send_s8(26, bytes([1]), "PAYLOAD_STANDBY")
        assert r["ack_11"]

    def test_cmd_pld_mode_imaging(self, harness):
        r = harness.send_s8(26, bytes([2]), "PAYLOAD_IMAGING")
        assert r["ack_11"]

    def test_cmd_pld_set_band_config(self, harness):
        r = harness.send_s8(33, bytes([0x0F]), "SET_BAND_ALL")
        assert r["ack_11"]

    def test_cmd_pld_set_detector_gain(self, harness):
        r = harness.send_s8(35, struct.pack('>f', 2.0), "SET_GAIN_2.0")
        assert r["ack_11"]

    def test_cmd_pld_set_cooler_setpoint(self, harness):
        r = harness.send_s8(36, struct.pack('>f', -30.0), "SET_COOLER_-30C")
        assert r["ack_11"]

    def test_cmd_pld_start_calibration(self, harness):
        r = harness.send_s8(37, name="START_CALIBRATION")
        assert r["ack_11"]

    def test_cmd_pld_stop_calibration(self, harness):
        r = harness.send_s8(38, name="STOP_CALIBRATION")
        assert r["ack_11"]

    def test_cmd_pld_set_compression(self, harness):
        r = harness.send_s8(39, struct.pack('>f', 4.0), "SET_COMPRESSION_4")
        assert r["ack_11"]

    def test_cmd_pld_capture_image(self, harness):
        r = harness.send_s8(28, struct.pack('>ff', 45.0, 10.0), "CAPTURE_45_10")
        assert r["ack_11"]

    def test_cmd_pld_download_image(self, harness):
        r = harness.send_s8(29, struct.pack('>H', 0), "DOWNLOAD_IMG_0")
        assert r["ack_11"]

    def test_cmd_pld_delete_image(self, harness):
        r = harness.send_s8(30, struct.pack('>H', 0), "DELETE_IMG_0")
        assert r["ack_11"]

    def test_cmd_pld_get_catalog(self, harness):
        r = harness.send_s8(32, name="GET_IMAGE_CATALOG")
        assert r["ack_11"]

    def test_cmd_pld_set_integration_time(self, harness):
        r = harness.send_s8(34, struct.pack('>ffff', 0.01, 0.02, 0.015, 0.025),
                            "SET_INT_TIMES")
        assert r["ack_11"]

    def test_cmd_pld_mark_bad_segment(self, harness):
        r = harness.send_s8(31, bytes([0]), "MARK_BAD_SEG_0")
        assert r["ack_11"]

    def test_cmd_pld_verify_hk(self, harness):
        """Verify Payload HK (SID 5) through RF."""
        hk = harness.get_hk(5)
...
class TestTCSCommandsRF:
    ...
    def test_cmd_tcs_heater_bat_on(self, harness):
        r = harness.send_s8(40, bytes([1]), "HTR_BAT_ON")
        assert r["ack_11"]

    def test_cmd_tcs_heater_bat_off(self, harness):
        r = harness.send_s8(40, bytes([0]), "HTR_BAT_OFF")
        assert r["ack_11"]

    def test_cmd_tcs_heater_obc_on(self, harness):
        r = harness.send_s8(41, bytes([1]), "HTR_OBC_ON")
        assert r["ack_11"]

    def test_cmd_tcs_fpa_cooler(self, harness):
        r = harness.send_s8(43, bytes([1]), "FPA_COOLER_ON")
        assert r["ack_11"]

    def test_cmd_tcs_set_setpoint(self, harness):
        data = bytes([1]) + struct.pack('>ff', 5.0, 10.0)
        r = harness.send_s8(44, data, "SET_SETPOINT_BAT")
        assert r["ack_11"]

    def test_cmd_tcs_auto_mode(self, harness):
        r = harness.send_s8(45, bytes([1]), "AUTO_MODE_BAT")
        assert r["ack_11"]

    def test_cmd_tcs_duty_limit(self, harness):
        r = harness.send_s8(46, bytes([1, 80]), "DUTY_LIMIT_80")
        assert r["ack_11"]

    def test_cmd_tcs_decontamination_start(self, harness):
        r = harness.send_s8(47, struct.pack('>f', 50.0), "DECON_START_50C")
        assert r["ack_11"]

    def test_cmd_tcs_decontamination_stop(self, harness):
        r = harness.send_s8(48, name="DECON_STOP")
        assert r["ack_11"]

    def test_cmd_tcs_thermal_map(self, harness):
        r = harness.send_s8(49, name="GET_THERMAL_MAP")
        assert r["ack_11"]

    def test_cmd_tcs_verify_hk(self, harness):
        """Verify TCS HK (SID 3) through RF."""
        hk = harness.get_hk(3)
...
class TestOBDHCommandsRF:
    ...
    def test_cmd_obd_set_mode(self, harness):
        r = harness.send_s8(50, bytes([1]), "OBC_MODE_NOMINAL")
        assert r["ack_11"]

    def test_cmd_obd_memory_scrub(self, harness):
        r = harness.send_s8(51, name="MEM_SCRUB")
        assert r["ack_11"]

    def test_cmd_obd_switch_unit(self, harness):
        r = harness.send_s8(53, name="OBC_SWITCH_UNIT")
        assert r["ack_11"]

    def test_cmd_obd_select_bus_a(self, harness):
        r = harness.send_s8(54, bytes([0]), "SELECT_BUS_A")
        assert r["ack_11"]

    def test_cmd_obd_boot_inhibit(self, harness):
        r = harness.send_s8(56, bytes([1]), "BOOT_INHIBIT_ON")
        assert r["ack_11"]
        # Disable inhibit for subsequent tests
        harness.send_s8(56, bytes([0]), "BOOT_INHIBIT_OFF")

    def test_cmd_obd_clear_reboot_cnt(self, harness):
        r = harness.send_s8(57, name="CLEAR_REBOOT_CNT")
        assert r["ack_11"]

    def test_cmd_obd_set_watchdog(self, harness):
        r = harness.send_s8(58, struct.pack('>H', 5000), "SET_WD_5000ms")
        assert r["ack_11"]

    def test_cmd_obd_watchdog_enable(self, harness):
        r = harness.send_s8(59, name="WD_ENABLE")
        assert r["ack_11"]

    def test_cmd_obd_watchdog_disable(self, harness):
        r = harness.send_s8(60, name="WD_DISABLE")
        assert r["ack_11"]

    def test_cmd_obd_diagnostic(self, harness):
        r = harness.send_s8(61, name="OBC_DIAGNOSTIC")
        assert r["ack_11"]

    def test_cmd_obd_error_log(self, harness):
        r = harness.send_s8(62, name="OBC_ERROR_LOG")
        assert r["ack_11"]

    def test_cmd_obd_gps_time_sync(self, harness):
        r = harness.send_s8(80, name="GPS_TIME_SYNC")
        assert r["ack_11"]

    def test_cmd_obd_verify_hk(self, harness):
        """Verify Platform HK (SID 4) through RF."""
        hk = harness.get_hk(4)
...
class TestConnectionTestRF:
    ...
    def test_beacon_hk_through_rf(self, harness):
        """Verify beacon HK (SID 11) through RF."""
